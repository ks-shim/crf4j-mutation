package io.github.ksshim.crf4j.mutation.trie;
...
public class DoubleArrayTrie implements Serializable {
    ...
    private static final long serialVersionUID = -8579120864363911644L;
    ...
    private int fetch(Node parent, List<Node> siblings) {
        ...
        for (int i = parent.left; i < parent.right; i++) {
            ...
            int depth = length != null ? length[i] : tmp.length();
            if(depth < parent.depth) continue;

            int cur = 0;
            if(depth != parent.depth) cur = (int) tmp.charAt(parent.depth) + 1;
    ...
    private int insert(List<Node> siblings) {
        ...
        int begin = 0;
        int pos =
                ((siblings.get(0).code + 1 > nextCheckPos) ? siblings.get(0).code + 1 : nextCheckPos) - 1;
        int nonzeroNum = 0;
        int first = 0;
    ...
    public void dump() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            sb.append("i: ").append(i)
                    .append(" [").append(base[i]).append(", ").append(check[i]).append("]");
